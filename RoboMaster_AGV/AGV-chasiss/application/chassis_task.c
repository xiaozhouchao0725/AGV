...
static void chassis_init(chassis_move_t *chassis_move_init)
{
	...
	// 斜坡函数初始化
	 ramp_init(&chassis_move_init->vx_ramp, 0.01f, 10, -10);
...
static void chassis_set_contorl(chassis_move_t *chassis_move_control)
{
	...
	if (chassis_move_control->chassis_motor_mode == CHASSIS_VECTOR_FOLLOW_GIMBAL_YAW) // 底盘跟随云台模式
	{
		...
		else
		{
			...
			// chassis_move_control->wz_set = -chassis_move_control->chassis_rc_ctrl->rc.ch[0] * 0.07;
		}
	}
...
//		else // 当原地时，加大转速
//		{
			chassis_move_control->wz_set = 40.0f;
//		}
	}
...
//rudder_optimal_angle->ecd_temp_error =	rudder_optimal_angle->ecd_set - rudder_optimal_angle->gimbal_motor_measure->ecd;
...
//	angle = (chassis_motor->ecd_set - chassis_motor->gimbal_motor_measure->ecd)*Motor_Ecd_to_Rad;
...
void CHASSIC_MOTOR_POWER_CONTROL(chassis_move_t *chassis_motor)
{
	...
	for (i = 0; i < 4; i++)
	{
		...
		chassis_motor->power_control.totalSpeed += fabs(chassis_motor->motor_chassis[i].speed);
	}

	chassis_motor->POWER_MAX_SET = chassis_motor->power_control.POWER_MAX; // 最大p处理，若使用功率计则进行相减
																		   // 使用功率计好处:更细节地控制作用  未使用原因:未来得及细调
	if (chassis_motor->POWER_MAX_SET < 0)
		chassis_motor->POWER_MAX_SET = 0;
	for (i = 0; i < 4; i++)
	{
		chassis_motor->power_control.MAX_current[i] = (chassis_motor->power_control.K 
		* chassis_motor->power_control.current[i] / chassis_motor->power_control.totalCurrentTemp) 
		* (chassis_motor->POWER_MAX_SET) 
		/ (fabs(chassis_motor->power_control.totalSpeed));
	}
	chassis_motor->power_control.totalCurrentTemp = 0;
	chassis_motor->power_control.totalSpeed =0;
...
void chassis_power_move_control(chassis_move_t *chassis_motor)
{
	...
	//			else
	//			{
	chassis_motor->power_control.K = 800;
